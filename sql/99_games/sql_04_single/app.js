...
function queueStairClimb() {
  ...
  if (!allCheckpointsCleared()) {
    tablePanelHintEl.textContent = "Staircase locked! Solve all checkpoints first.";
    return;
  }
...
function handleLanding() {
  ...
  if (player.x === config.stair.x && player.y === config.stair.y) {
    if (currentFloor === TROPHY_FLOOR_INDEX && !hasTrophy) {
      clearPendingStairClimb();
      tablePanelHintEl.textContent = "Collect the trophy first before using this staircase.";
      return;
    }
    if (!allCheckpointsCleared()) {
      clearPendingStairClimb();
...
// Preload questions and start the game immediately
ensureQuestionBankLoaded().then(() => {
  resetGame();
