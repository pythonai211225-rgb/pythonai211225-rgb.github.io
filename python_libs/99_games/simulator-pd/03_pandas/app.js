...
/* ── deep-link: open the panel named in the URL hash ──────────
   Supports #panel-N (opens sub-topic panel N) or #some-id
   (opens the panel that contains the element and scrolls to it).
   Used by the Pandas Simulator "Open Topic" button. */
function openHashPanel() {
  const id = decodeURIComponent((location.hash || '').slice(1));
  if (!id || typeof window.goTo !== 'function') return;
  const el = document.getElementById(id);
  const panel = el && el.closest ? el.closest('.panel') : null;
  if (!panel) return;
  const idx = parseInt(panel.id.replace('panel-', ''), 10);
  if (!Number.isNaN(idx)) window.goTo(idx);
  if (el !== panel) {
    // wait for the panel/section fade-in (~0.55s) to settle, then scroll to it
    setTimeout(() => el.scrollIntoView({ behavior: 'smooth', block: 'start' }), 680);
  }
}
window.addEventListener('load', openHashPanel);
